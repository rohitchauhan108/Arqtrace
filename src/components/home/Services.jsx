      ...
      <div className="container mx-auto px-6 lg:px-16 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-14 gap-6">
          <div className="flex items-start gap-4">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold text-[#2d1e18] leading-[1.15] tracking-tight">
              Premium Products for Every
              <br /> Window, Door & Space
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            <div
              key={index}
              className="group bg-white rounded-none p-6 shadow-sm hover:shadow-2xl hover:-translate-y-2 hover:bg-[#bd845c] transition-all duration-500 ease-out flex flex-col justify-between h-full min-h-[460px] border border-stone-200/40 relative"
            >
              <div>
                <div className="relative aspect-[4/3] w-full overflow-hidden mb-6 bg-stone-50">
                  <img
                    src={service.image}
                    alt={service.title}
                    className="w-full h-full object-cover grayscale-[30%] group-hover:grayscale-0 group-hover:scale-105 transition-transform duration-700 ease-out"
                  />
                ...
                <h3 className="text-lg font-serif font-bold text-[#2d1e18] mb-3 group-hover:text-white transition-colors duration-300">
                  {service.title}
                </h3>
                <p className="text-stone-500 text-sm leading-relaxed mb-6 group-hover:text-white/80 transition-colors">
                  {service.description}
                </p>
              </div>
